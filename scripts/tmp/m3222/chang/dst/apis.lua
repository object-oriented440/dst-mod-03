...
function API:namedPrefab()
    local name = API:getFileNameNoExtension(3); -- xxx.chang.dst
    local folder_name = getupfilename();
...
function API:loadPrefabs(filepaths)
    ...
    for _, filename in ipairs(filepaths) do
        ...
        if (not f) then
            ...
            for _, prefab in pairs({ Load:changxpcall(f) }) do
                ...
                for _, c in ipairs(vip_containers) do
                    ...
                    if (container and container:IsOpen()) then
                        if (container:GiveItem(inst, nil, src_pos)) then
                            ...
                            ((TheFocalPoint or F).SoundEmitter or F):PlaySound("dontstarve/HUD/collect_resource")
                            return true;
                        end
                    end
                end
