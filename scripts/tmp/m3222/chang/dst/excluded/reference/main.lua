...
--[[
	...
	luaL_loadfile(L, filename); -- 将 Lua 的所有代码段 当做匿名函数压栈，此时栈顶已存在一个函数

	... 此处可以注册 C 函数、全局变量进 Lua 中
	比如：
	-- lua_pushcfunction(L, get_absolute_path);
	-- lua_setglobal(L, "get_absolute_path");
	此时，Lua 中可以直接调用 get_absolute_path 函数了！

	lua_pcall(L, 0, 0, 0); -- 执行被压入栈顶的函数

	-- lua_getglobal(L, "Lua 函数名");
	-- 通过这种方式，可以获取 Lua 中的全局变量，压入虚拟栈中！
	-- 然后：lua_pcall(L, args_number, results_number, 0) 直接在 C 中调用这个函数！
]]
TheSim:SetReverbPreset("default") -- 所以有理由猜测，TheSim 是底层 C 注入到 Lua 中的！
-- 也就是说，全局搜索找不到的声明位置的全局变量！都可以认为是封装在底层的！
...
-- NOTE 在此 entityscript 之前，TheWorld 全局变量已存在 TODO:在哪加载 mod 和 prefabs 的？
-- NOTE prefabs 文件夹下显然 TheWorld 全局变量是存在的！
require("profiler")
require("recipes")
require("brain")
require("emitters")
