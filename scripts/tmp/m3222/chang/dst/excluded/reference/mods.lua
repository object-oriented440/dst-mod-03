...
function GetEnabledModNamesDetailed()
    --just used for callstack reporting
    local name_details = {}
    ...
    return (function(...)
        if fn then
            ...
            if not status then
                ...
                return r -- fn(unpack(arg)) 的函数执行后的返回结果
            end
        end
    end)
end
...
function ModWrangler:InitializeModMain(modname, env, mainfile, safe)
    if not KnownModIndex:IsModCompatibleWithMode(modname) then
        return
    end
    ...
    if TheFrontEnd then
        for k, badmod in ipairs(self.failedmods) do
            SetGlobalErrorWidget(
                    ...
                    {
                        { text = STRINGS.UI.MAINSCREEN.SCRIPTERRORQUIT, cb = function()
                            TheSim:ForceAbort()
                        end },
    ...
    for i, modname in ipairs(self.enabledmods) do
        ...
        if mod.PrefabFiles then
            for _, prefab_path in ipairs(mod.PrefabFiles) do
                ...
                -- DST 就当前位置而言，ret 就是 LoadPrefabFile 函数的执行结果
                -- ret = { f() } f() 就是你加载的 prefabs 目录下文件的返回值！
                if ret then
                    for _, prefab in ipairs(ret) do -- ipairs 会跳过 nil 值！
        ...
        if not DISABLE_MOD_WARNING and IsInFrontEnd() then
            TheFrontEnd:PushScreen(
                    ModWarningScreen(
                            ...
                            moddetail,
                            {
                                { text = STRINGS.UI.MAINSCREEN.TESTINGYES, cb = function()
...
---此时的注释就 entityscript.lua|function EntityScript:AddComponent(name)
---local postinitfns = ModManager:GetPostInitFns("ComponentPostInit", name)
---而言，已经传入了 type,id = "ComponentPostInit", name
---懂了，这意思是：只要 inst:AddComponent(name) 函数被调用了，就执行一次所有 mod
---的 ComponentPostInit 函数！！！
function ModWrangler:GetPostInitFns(type, id)
    local retfns = {}
    for i, modname in ipairs(self.enabledmods) do -- 按照加载顺序遍历！所以会依次覆盖！
        -- 遍历已开启的所有 mod
        local mod = self:GetMod(modname) -- 根据 mod 名获取 mod
