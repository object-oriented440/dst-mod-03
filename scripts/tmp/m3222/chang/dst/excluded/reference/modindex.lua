...
package.preload["moddependencymanager"] = function()
end
package.preload["librarymanager"] = function()
    return function(_dependencies)
        ...
        if not rawget(_G, "TheFrontEnd") or (not _dependencies or not modenv or not modenv.modname or (KnownModIndex.savedata.known_mods[modenv.modname] and
                KnownModIndex.savedata.known_mods[modenv.modname].dependencies)) then
            return
        end
        local dependencies = {}
...
function ModIndex:ApplyEnabledOverrides(mod_overrides)
    --Note(Peter): This function is now coupled with the format written by ShardSaveIndex:SetSlotEnabledServerMods
...
function ModIndex:LoadModInfo(modname, prev_info)
    ...
    if prev_info ~= nil and prev_info.version == info.version then
        return prev_info
    end
    ...
    if info.mod_dependencies and not info.client_only_mod then
        --todo(Zachary): support client mods in the future
        local dependencies = {}
...
function ModIndex:GetModFancyName(modname)
    ...
    -- 根据 Lua 程序设计这本书 改进一下，虽然好像用处不大
    if ((knownmod or {}).modinfo or {}).name then
        return knownmod.modinfo.name
    else
        return modname
    end
    ...
    if known_mod then
        if known_mod.modinfo then
            if known_mod.modinfo.configuration_options then
                ...
                end, modname, known_mod.modinfo.configuration_options, false)
            end
        end
    end
end
...
function ModIndex:DisableClientMods(disabled)
    --to be called from the server/host when loading mod overrides. Note, this is not saved out to the mod index file on the clients, it's applied through the server listing temp disabling individual client mods when connecting
...
-- NOTE:KnownModIndex 为 ModIndex 的实例！继承（浅拷贝）了 ModIndex 的所有内容！
-- NOTE:因为是浅拷贝，所以只要你修改了，就和该原型无关了！
