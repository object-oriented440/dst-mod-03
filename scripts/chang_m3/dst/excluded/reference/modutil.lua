...
local function InsertPostInitFunctions(env, isworldgen, isfrontend)
	...
	env.AddComponentPostInit = function(component, fn)
		...
		-- 每次调用 AddComponentPostInit 函数时，都会往
		-- env.postinitfns.ComponentPostInit[component] 这个表中插入添加的函数！
		-- 在 modutil.lua function ModWrangler:GetPostInitFns(type, id) 发现，
		-- 这些函数在 inst:AddComponent(name) 函数被调用时！全部执行一遍！
