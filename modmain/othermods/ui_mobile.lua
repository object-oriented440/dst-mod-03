...
local SaveDragPos = function(dragtype, pos)
    if next(dragpos) then
        ...
        -- FIXME:
        TheSim:SetPersistentString("acb_drag_pos", str, false)
    end
end
...
local MakeDragableUI = function(self, dragtarget, dragtype, dragdata)
    ...
    --给拖拽目标添加拖拽提示
    if dragtarget then
        dragtarget:SetTooltip("按住右键即可拖拽")
...
local ResetUIPos = function()
    dragpos = {}
    -- FIXME:
    TheSim:SetPersistentString("acb_drag_pos", "", false)
